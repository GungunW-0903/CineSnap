        ...
        <div key={index} className='flex flex-col md:flex-row justify-between bg-[#f84565]/8 border border-[#f84565]/20 rounded-lg mt-4 p-2 max-w-3xl '>
          <div className='flex flex-col md:flex-row'>
            ...
            <div className='flex flex-col mt-4 px-4'>
              <p className='text-lg font-bold'>{item.show.movie.title}</p>
          ...
          <div className='flex flex-col md:items-end md:text-right p-4'>
            <div className='flex items-center gap-4'>
              ...
              {!item.isPaid && <button className='bg-[#f84565] px-4 py-2 text-sm rounded-full font-medium cursor-pointer'>Pay Now</button>}
            </div>
            <div>
              <p ><span className='text-gray-400 text-sm gap-2'>Total Tickets:  </span>{item.bookedSeats.length}</p>
              <p><span  className='text-gray-400 text-sm gap-2'>Seat Number:  </span>{item.bookedSeats.join(", ")}</p>
            </div>
          </div>
